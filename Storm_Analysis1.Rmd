...
---
title: "Storm & Weather Event Analysis"
output: 
  html_document:
    keep_md: true
---
...
```{r "read_and_clean_data", cache=TRUE}
...
```{r, "cleaner", echo=TRUE, cache=TRUE}
...
The reduction above of the data into "impactful severe events" reduced the number of elements from `r numbah1[1]` to `r numbah2[1]` line items.  
...
```{r "event_extractions", cache=TRUE}
...
The classification results in `r numbahd[1]` rows of data.  
The table below is a small random subset of the final data.

```{r "show_me_the_data", echo=TRUE, results="asis"}
...
```{r "Severity", echo=TRUE, cache=TRUE}
...
```{r "multiplot_graph", echo=TRUE, fig.height=6, fig.width=12 }
...
```{r "damage_total_Multiplot", echo=TRUE, fig.height=6, fig.width=12 }
...
```{r "time_series", echo=TRUE, fig.height=6, fig.width=12 }
...
```{r, "plot_the_stupid_thing", echo=TRUE, fig.height=6, fig.width=12}
